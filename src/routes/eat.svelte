...
{:else}
	...
	<div class='ate-wrapper mt-1'>
		{#each foodIAte as { id, name, count }, i}
			...
			<div>{name}</div>
			<input class="bg-gray-200 w-6" maxlength="3" min="1" max="999" type="number" on:keyup={(e) => handleEdit(foodIAte[i], e.target.value)} value={count} />
		{/each}
	</div>
	<ul>
		<li class="first"><a href="/food">Choose Food</a></li>
		<li class="second">Ate Today</li>
	</ul>
{/if}
